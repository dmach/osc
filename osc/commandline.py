...
#                    sys.exit(1)
    ...
    def do_results(self, subcmd, opts, *args):
        ...
        if opts.multibuild_package and opts.no_multibuild:
            raise self.argparser.error("-M/--multibuild-package and --no-multibuild are mutually exclusive")

        if opts.xml and opts.format:
            raise self.argparser.error("--xml and --format are mutually exclusive")
